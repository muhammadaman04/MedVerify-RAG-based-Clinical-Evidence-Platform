...
class BM25Index:
    """
    In-memory BM25 index over document chunks for a single organization.
    Call `rebuild(org_id)` after each new document is ingested.
    """
    def __init__(self) -> None:
        self._bm25 = None
        self._chunk_ids: list[str] = []
        self._pinecone_ids: list[str] = []
    ...
    def rebuild(self, org_id: str) -> None:
        """
        Fetch chunks for a specific org from Supabase and rebuild the BM25 index.
        Called after document ingestion and at startup for each known org.
        """
        ...
        if not rows:
            print(f"⚠️  BM25 [{org_id[:8]}]: No chunks found. Index is empty.")
            self._bm25 = None
            return
        ...
        print(f"✓ BM25 [{org_id[:8]}] rebuilt with {len(rows)} chunks.")
...
# Per-org BM25 index registry: {org_id: BM25Index}
_bm25_indexes: dict[str, BM25Index] = {}
_bm25_lock = __import__("threading").Lock()


def get_bm25_index(org_id: str) -> BM25Index:
    """Return (or lazily create) the BM25 index for the given org."""
    if org_id not in _bm25_indexes:
        with _bm25_lock:
            if org_id not in _bm25_indexes:
                idx = BM25Index()
                idx.rebuild(org_id)
                _bm25_indexes[org_id] = idx
    return _bm25_indexes[org_id]


def rebuild_bm25_index(org_id: str) -> None:
    """
    Rebuild (or create) the BM25 index for a specific org.
    Called after new document ingestion and at startup.
    """
    with _bm25_lock:
        if org_id not in _bm25_indexes:
            _bm25_indexes[org_id] = BM25Index()
        _bm25_indexes[org_id].rebuild(org_id)
...
def vector_search(query: str, org_id: str, top_k: int = 20) -> list[dict]:
    """
    3.2 — Vector search.
    Embeds the query and queries Pinecone WITHIN the org's namespace.
    ...
    results = index.query(
        ...
        namespace=org_id,            # ← tenant-isolated namespace
    )
...
def hydrate_chunks(fused_results: list[dict], org_id: str) -> list[RetrievedChunk]:
    """
    Given RRF-fused results, fetch raw_text from Supabase document_chunks
    filtered by org_id so a tenant never sees another tenant's data.
    """
    ...
    result = (
        db.table("document_chunks")
        .select("id, pinecone_vector_id, raw_text, page_number, heading_context, document_id")
...
def hybrid_retrieve(query: str, org_id: str, top_k: int = 20) -> list[RetrievedChunk]:
    """
    Full hybrid retrieval pipeline, org-scoped:
      1. Vector search in org's Pinecone namespace
      2. BM25 keyword search on org's chunks
      3. Reciprocal Rank Fusion
      4. Hydrate from Supabase (org-filtered)
    """
    vec_results  = vector_search(query, org_id=org_id, top_k=top_k)
    bm25_results = get_bm25_index(org_id).search(query, top_k=top_k)
    fused        = reciprocal_rank_fusion(vec_results, bm25_results, top_k=top_k)
    return hydrate_chunks(fused, org_id=org_id)
